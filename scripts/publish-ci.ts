...
// How to test the release scripts locally:
// RELEASE_VERSION=5.0.0-beta.6 pnpm release
// PUBLISH_DRY_RUN=true PUBLISH_BRANCH=main pnpm publish-ci 5.0.0-beta.6
...
async function main() {
  ...
  await $$`pnpm stage publish --access public --no-git-checks --tag ${releaseTag} ${dryRun ? ['--dry-run'] : []}`
}
...
async function getReleaseTag(version: string, publishBranch: string) {
  ...
  const npmView = await $`npm view @vitest/browser-webdriverio dist-tags --json`
